...
# API Token
if [ -f /etc/zuluwarrior/token.conf ]
then
	auth=($(cat /etc/zuluwarrior/token.conf))
else
	echo "Error: File /etc/zuluwarrior/token.conf is missing."
	exit 1
fi
...
if [ -e /etc/zuluwarrior/cache ]
then
	data=($(cat /etc/zuluwarrior/cache))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/zuluwarrior/cache
...
# Get network latency
ping_eu=$(prep $(num "$(ping -c 2 -w 2 ping-de-1.rev.zuluwarrior.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_us=$(prep $(num "$(ping -c 2 -w 2 ping-us-1.rev.zuluwarrior.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_as=$(prep $(num "$(ping -c 2 -w 2 ping-as-1.rev.zuluwarrior.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/zuluwarrior/log/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.zuluwarrior.net/agent/report"
else
	wget -q -o /dev/null -O /etc/zuluwarrior/log/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.zuluwarrior.net/agent/report"
	wget_pid=$!
	wget_counter=0
	wget_timeout=30
