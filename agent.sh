...
# API Token
if [ -f /etc/cybex/token.conf ]
then
	auth=($(cat /etc/cybex/token.conf))
else
	echo "Error: File /etc/cybex/token.conf is missing."
	exit 1
fi
...
if [ -e /etc/cybex/cache ]
then
	data=($(cat /etc/cybex/cache))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/cybex/cache
...
# Get network latency
ping_eu=$(prep $(num "$(ping -c 2 -w 2 ping-de-1.rev.cybex.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_us=$(prep $(num "$(ping -c 2 -w 2 ping-us-1.rev.cybex.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_as=$(prep $(num "$(ping -c 2 -w 2 ping-as-1.rev.cybex.net | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/cybex/log/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.cybex.net/agent/report"
else
	wget -q -o /dev/null -O /etc/cybex/log/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.cybex.net/agent/report"
	wget_pid=$!
	wget_counter=0
	wget_timeout=30
